...
predictor = dlib.shape_predictor(
    path.join(file_dir, "data", "shape_predictor_68_face_landmarks.dat")
)
...
def get_largest_face(dets):
    ...
    face_areas = [
        (det.right() - det.left()) * (det.bottom() - det.top()) for det in dets
    ]
...
if __name__ == "__main__":
    ...
    KalmanX = KalmanObject(
        POINTS_NUM_LANDMARK, 1, 10
    )  # Tune Q, R to change landmarks_x sensitivity
    KalmanY = KalmanObject(
        POINTS_NUM_LANDMARK, 1, 10
    ...
    while cap.isOpened():
        ...
        landmarks = mean_filter_for_landmarks(
            landmarks
        ...
        parameters_str = (
            "leftEyeWid:{}, rightEyewid:{}, mouthWid:{}, mouthLen:{}".format(
        ...
        data = (
            str(translation_vector[0, 0])
            + ":"
            + str(translation_vector[1, 0])
            + ":"
            + str(translation_vector[2, 0])
            + ":"
            + str(w)
            + ":"
            + str(x)
        ...
        for i in range(8):
            (boxPoints2D[:, :, i, :], jacobian) = cv2.projectPoints(
                np.array([boxPoints3D[i]]),
                rotation_vector,
                translation_vector,
                camera_matrix,
                dist_coeffs,
